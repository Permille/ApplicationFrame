	...
	constructor(GivenApplicationProperties = {}){
		...
		/*let Validator = {
			"set": function(Obj, Property, Value){
				switch(Property){
					case "Width":
						//if(this?.Properties?.Dimensions?.Minimum?.Width === undefined);
						if(!(this.Properties && this.Properties.Dimensions && this.Properties.Dimensions.Minimum && this.Properties.Dimensions.Minimum.Width));
						else if(Value < this.Properties.Dimensions.Minimum.Width) Value = this.Properties.Dimensions.Minimum.Width;
						else if(!(this.Properties && this.Properties.Dimensions && this.Properties.Dimensions.Minimum && this.Properties.Dimensions.Maximum.Width));
						else if (Value > this.Properties.Dimensions.Maximum.Width) Value = this.Properties.Dimensions.Maximum.Width;
						Obj.__Width__ = Value;
						this.WrapperElement.style.width = Value + "px";
						break;
					case "Height":
						//if(this?.Properties?.Dimensions?.Minimum?.Height === undefined);
						if(!(this.Properties && this.Properties.Dimensions && this.Properties.Dimensions.Minimum && this.Properties.Dimensions.Minimum.Height));
						else if(Value < this.Properties.Dimensions.Minimum.Height) Value = this.Properties.Dimensions.Minimum.Height;
						else if(!(this.Properties && this.Properties.Dimensions && this.Properties.Dimensions.Maximum && this.Properties.Dimensions.Maximum.Height));
						else if (Value > this.Properties.Dimensions.Maximum.Height) Value = this.Properties.Dimensions.Maximum.Height;
						Obj.__Height__ = Value;
						this.WrapperElement.style.height = Value + "px";
						break;
				}
				return true;
			}.bind(this),
			...
			"Validator":{
				...
				"set": function(Obj, Property, Value){
					switch(Property){
						case "XPos":
							Obj.__XPos__ = Value;
							Scope.WrapperElement.style.left = Value + "px";
							return true;
						case "YPos":
							Obj.__YPos__ = Value;
							Scope.WrapperElement.style.left = Value + "px";
						...
						case "YPos":
							...
							return true;
						default:
							Obj["__" + Property + "__"] = Value;
							return true;
					}
				}.bind(this)
			},
			"Dimensions":{
				///Leaving LockAspectRatio as it is because it doesn't really need to trigger anything
				"Current": {
					"set": function(Obj, Property, Value){
						switch(Property){
							case "Width":
		...
		/*"Border": {
			"get": function(Obj, Property){
				switch(Property){
					case "Width":
						return Obj.__Width__;
					case "TransparentBorderWidth":
						return Obj.__TransparentBorderWidth__;
				}
			}.bind(this),
			"set": function(Obj, Property, Value){
				switch(Property){
					case "Width":
						Obj.__Width__ = Value;
						this.Elements.TableElement.style.border = Value + "px solid black";
						break;
					case "TransparentBorderWidth":
						Obj.__TransparentBorderWidth__ = Value;
						break;
				}
				return true;
			}.bind(this)
		...
		//PropertiesTemplate.Dimensions.Current = new Proxy(PropertiesTemplate.Dimensions.Current, Validator);
